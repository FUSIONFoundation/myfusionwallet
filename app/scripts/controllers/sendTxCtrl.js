...
var sendTxCtrl = function ($scope, $sce, walletService, $rootScope) {
        ...
        $scope.$watch("timeLockList", function () {
            if (typeof $scope.timeLockList === "undefined") {
                ...
                $scope.$eval(function () {
                    $scope.endPage = Math.ceil(
                        $scope.timeLockList.length / $scope.pageSize
                    );
                });
            }
        });
        ...
        $scope.manageAssetOpen = async function (id) {
            ...
            let hasUrlCondition = urlconditions.some(el =>
                $scope.assetListOwns[id].description.toString().includes(el)
            );
        ...
        $scope.changeSupplyTx = async function () {
            ...
            if ($scope.changeSupplyState == "decrement") {
                // Get New Total Supply, create BN and create Hexadecimal
                let bal =
                    $scope.assetListOwns[$scope.lastId].total - $scope.newTotalSupply;
                let newtotalSupplyString = bal.toString();
                let newtotalSupplyBN = $scope.makeBigNumber(
                    newtotalSupplyString,
                    asset.decimals
        ...
        $scope.createAssetErrorHandler = function () {
            ...
            if (
                ...
                $scope.assetCreate.assetName.length > 35
            ) {
            ...
            if (
                ...
                $scope.assetCreate.assetSymbol.length > 4
            ) {
                $scope.createAssetHasError = true;
                return;
            }
            ...
            if (
                ...
                $scope.assetCreate.totalSupply <= 0
            ) {
                $scope.createAssetHasError = true;
                return;
            }
        ...
        $scope.onlyLettersAndNumbersAssetName = function () {
            ...
            if (!reg.test($scope.assetCreate.assetName)) {
                $scope.$eval(function () {
                    $scope.assetCreate.assetName = $scope.assetCreate.assetName.replace(
                        /[^a-z0-9 ,.?!]/gi,
                        ""
                    );
                });
        ...
        $scope.onlyLettersAndNumbersAssetSymbol = function () {
            ...
            if (!reg.test($scope.assetCreate.assetSymbol)) {
                $scope.$eval(function () {
                    $scope.assetCreate.assetSymbol = $scope.assetCreate.assetSymbol.replace(
                        /[^a-z0-9 ,.?!]/gi,
                        ""
                    );
                });
        ...
        $scope.getTimeLockStatus = function (
            startTime,
            endTime,
            startTimePosix,
            endTimePosix
            ...
            for (let asset in timeLockList) {
                ...
                for (let i = 0; i < timeLockList[asset]["Items"].length; i++) {
                    ...
                    if (startTimePosix === 0) {
                        ...
                        let a = new Date(
                            timeLockList[asset]["Items"][i]["StartTime"] * 1000 + 1000
                        );
                    ...
                    if (endTimePosix === "18446744073709551615") {
                        ...
                        let a = new Date(
                            timeLockList[asset]["Items"][i]["EndTime"] * 1000 + 1000
                        );
                    ...
                    status = $scope.getTimeLockStatus(
                        startTime,
                        endTime,
                        startTimePosix,
                        endTimePosix
